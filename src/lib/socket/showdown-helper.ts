...
import { HandRank } from '../poker/evaluator'
import type { HandEndedData } from './game-types'
import type { ShowdownPayload, ShowdownPotResult, ShowdownPlayerResult } from './types'
...
export function computeShowdown(tableId: string, data: HandEndedData): ShowdownPayload {
  ...
  const players: ShowdownPlayerResult[] = data.players.map(p => {
    ...
    return {
      ...
      hasFolded: p.hasFolded,
      // Only reveal cards when it went to showdown and the player didn't fold.
      holeCards: data.reason === 'showdown' && !p.hasFolded ? p.holeCards : null,
    }
  })

  return { tableId, reason: data.reason, pots, players, communityCards: data.communityCards }
}
